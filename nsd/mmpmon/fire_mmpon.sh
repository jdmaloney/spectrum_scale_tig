...
sudo /usr/lpp/mmfs/bin/mmpmon -i /etc/telegraf/mmpmon/infile_clients -p | awk -v day="$day" '!/_rc_ 0/ {next;} /fs_io_s/ {print "gpfsperf,fs="$15",day="day",node="$5" br="$19",bw="$21",oc="$23",cc="$25",rr="$27",wr="$29}'
...
/_nsd_ds_/ {
   ...
   printf "gpfsnsdperf,fs=%s,day=%s,node=%s,nsd=%s rops=%s,wops=%s,rbytes=%s,wbytes=%s\n",nsds[nsd],day,node,nsd,rops,wops,rbytes,wbytes;
